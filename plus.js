...
(() => {
  ...
  function copyFor(state) {
    ...
    return { title:"Plano Convencional", text:"As ferramentas essenciais de estudo continuam disponíveis gratuitamente.", detail:state.trialUsed ? "Seu teste gratuito já foi utilizado." : "Você pode experimentar o Plus por 7 dias." };
  }
  ...
  function render() {
    ...
    main.innerHTML = `<div class="plus-page">
      ...
      <section class="plus-section"><div class="plus-section-head"><div><h3>Dois planos, o mesmo objetivo educacional</h3><p>O conteúdo essencial continua acessível para todos; o Plus adiciona ferramentas extras.</p></div><span class="plus-pill">VERSÃO TCC</span></div>
        ...
        <article class="plus-plan plus-plan--plus ${active ? "plus-plan--selected" : ""}"><span class="plus-plan__badge">PLUS</span><h4>M.E.N.T.E Plus</h4><div class="plus-plan__price"><strong>7 dias</strong><span>teste demonstrativo</span></div><p>Para quem quer uma experiência mais personalizada e orientada pelos próprios erros.</p><ul class="plus-feature-list"><li>Tudo do Convencional</li><li>Avatares especiais</li><li>Simulado personalizável</li><li>Diagnóstico avançado</li><li>Revisão Inteligente</li></ul></article></div></section>
      ...
      <div class="plus-note"><strong>Importante para o TCC:</strong> não há cobrança real. O Supabase M.E.N.T.E 2 controla o plano, a validade e o teste de 7 dias; a arquitetura fica pronta para uma integração futura com pagamento.</div>
    </div>`;
  ...
  async function activateTrial() {
    ...
    button.disabled = true; feedback.textContent = "Ativando seu teste Plus..."; feedback.classList.remove("is-error");
    try {
      ...
      feedback.textContent = "M.E.N.T.E Plus ativado!";
    } catch (error) {
      feedback.classList.add("is-error"); feedback.textContent = error?.message || "Não foi possível ativar o teste agora."; button.disabled = false;
    }
  }
