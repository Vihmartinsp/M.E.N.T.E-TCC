...
(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .admin-plus-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin:18px 0}.admin-plus-card{padding:18px;border:1px solid #dfe6ef;border-radius:16px;background:#fff}.admin-plus-card small{display:block;color:#7a8798;font-size:10px;font-weight:900;text-transform:uppercase}.admin-plus-card strong{display:block;margin-top:6px;color:#17213a;font-size:24px}.admin-plus-card span{display:block;margin-top:4px;color:#68758a;font-size:11px}.admin-plan-select{min-width:150px;height:36px;padding:0 28px 0 10px;border:1px solid #d9e2ec;border-radius:9px;background:#fff;color:#334155;font:inherit;font-size:11px;font-weight:700}.admin-plan-chip{display:inline-flex;align-items:center;padding:5px 9px;border-radius:999px;font-size:10px;font-weight:900}.admin-plan-chip--plus{background:#fff4bf;color:#725300}.admin-plan-chip--free{background:#eef2f7;color:#536174}.admin-plus-note{margin:14px 0;padding:12px 14px;border:1px solid #e5dcff;border-radius:12px;background:#faf8ff;color:#5b3fb7;font-size:12px;line-height:1.5}.admin-role-select option[value="professor"]{display:none!important}@media(max-width:900px){.admin-plus-summary{grid-template-columns:1fr 1fr}}@media(max-width:600px){.admin-plus-summary{grid-template-columns:1fr}}
    `;
  ...
  async function loadProfiles() {
    const { data, error } = await client.from("profiles").select("id,nome,email,plano,plus_iniciado_em,plus_expira_em,plus_teste_usado,ultimo_acesso").order("created_at", { ascending:true });
    if (error) throw error;
    profiles = data || [];
  }

  function activePlus(profile) {
    return profile.plano === "plus" && (!profile.plus_expira_em || Date.parse(profile.plus_expira_em) > Date.now());
  }
  ...
  function renderPanel() {
    ...
    panel.innerHTML = `
      <div class="admin-intro"><div><span class="admin-kicker">M.E.N.T.E Plus</span><h2>Planos e recursos premium</h2><p>Acompanhe quais contas usam o plano convencional ou Plus e gerencie os acessos de demonstração do TCC.</p></div></div>
      <div class="admin-plus-summary">
        ...
        <article class="admin-plus-card"><small>Testes usados</small><strong>${trials}</strong><span>testes de 7 dias já ativados</span></article>
      ...
      <div class="admin-plus-note">👑 Como Super Admin, você também possui o seletor de prévia no topo da plataforma. Ele alterna entre Convencional e Plus só para visualização e não muda o plano real da sua conta.</div>
      <article class="admin-panel admin-table-panel"><div class="admin-panel__head"><div><span>Controle de planos</span><h3>Usuários</h3></div><strong>${profiles.length} conta(s)</strong></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Pessoa</th><th>Plano atual</th><th>Início Plus</th><th>Expira em</th><th>Teste usado</th><th>Alterar plano</th></tr></thead><tbody>${profiles.map((p) => {
        const plusOn = activePlus(p);
        return `<tr><td><div class="admin-person"><span class="admin-person__avatar">${esc((p.nome || p.email || "A").slice(0,1).toUpperCase())}</span><div><strong>${esc(p.nome || "Aluno")}</strong><span>${esc(p.email || "—")}</span></div></div></td><td><span class="admin-plan-chip ${plusOn ? "admin-plan-chip--plus" : "admin-plan-chip--free"}">${plusOn ? "★ Plus" : "Convencional"}</span></td><td>${fmt(p.plus_iniciado_em)}</td><td>${fmt(p.plus_expira_em)}</td><td>${p.plus_teste_usado ? "Sim" : "Não"}</td><td>${currentRole === "super_admin" ? `<select class="admin-plan-select" data-plan-user="${p.id}"><option value="convencional" ${plusOn ? "" : "selected"}>Convencional</option><option value="plus30" ${plusOn ? "selected" : ""}>Plus · 30 dias</option><option value="plus90">Plus · 90 dias</option></select>` : "Somente Super Admin"}</td></tr>`;
